...
export class ChangeTrackableProxy extends Function {
     ...
     * @returns {boolean} 
     */
    removeAttribute(keyOrProperty, key) {
        if (key === undefined) {
            return this.attributes.delete(keyOrProperty);
        } else if (this.propAttributes.has(keyOrProperty)) {
            const a = this.propAttributes.get(keyOrProperty);
            return a.delete(key);
        }
     ...
     * @returns {boolean} 
     */
    static removeAttribute(proxy, keyOrProperty, key) {
        return proxy().removeAttribute(keyOrProperty, key);
    }
}
